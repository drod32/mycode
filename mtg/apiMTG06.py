...
def main():
    ...
    card_sets = response.json().get("sets")
    # open a file we write our data into
    with open("mtgsets.index", "w") as mtgfile:
        for card_set in card_sets:  # loop across ALL of the sets of MTG cards (they are stored as dict objects)
            print(f"{card_set.get('name')} -- {card_set.get('code')}", file=mtgfile)  # write the data "we want"
                                                                                    # into mtgfile
