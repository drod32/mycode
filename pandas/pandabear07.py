...
""" Alta3 Research
Creating a Datafranme from a time series dataset
and working with methods to display various network usage."""

import pandas as panda

def main():

    opsd_daily = panda.read_csv("netTraffic.csv") # this opens the csv dataset

    print(opsd_daily.shape)
    # (4383, 5) # our data has 4383 rows covering january 1,2006 through Dec 31, 2017

    print("\nLook at the last three rows")
    print(opsd_daily.head(3))

    print("\nLook at th last three rows")
    print(opsd_daily.tail(3))

    # check out data types of each column
    print(opsd_daily.dtypes)

    # set the date as the dataframes index 
